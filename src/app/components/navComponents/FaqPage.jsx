...
const FaqPage = () => {
  ...
  const { cms, loading } = usePublicCms();

  // Helper to safely read CMS text fields
  const getText = (field, lang) => {
    return field?.[lang] || field?.en || "";
  };

  // Translation function — reads from CMS faqPage
  const t = (key) => {
    const field = cms?.faqPage?.[key];
    return getText(field, language);
  };

  // Tag map from CMS — memoized so useMemo below stays stable
  const tagMap = useMemo(
    () => ({
      account: t("tagAccount"),
      savings: t("tagSavings"),
      plans: t("tagPlans"),
      circles: t("tagCircles"),
      islamic: t("tagIslamic"),
      security: t("tagSecurity"),
    }),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [cms?.faqPage, language]
  );
    ...
    () =>
      (cms?.faq || [])
        ...
        .map((item, index) => ({
          ...
          tag: tagMap[item.category] || tagMap.account,
    ...
    <div className="min-h-screen bg-[#f8fafc] font-['Segoe_UI',system-ui,sans-serif] text-[#0f172a] dark:bg-[#0a0f1e] dark:text-[#f1f5f9]">
      ...
      <section className="relative overflow-hidden bg-[linear-gradient(135deg,#059669,#0891b2)] px-6 pb-20 pt-16 text-center">
        ...
        <div className="absolute bottom-0 left-0 right-0">
          ...
          >
            <path
              className="fill-[#f8fafc] dark:fill-[#0a0f1e]"
      ...
      <div className="relative z-10 mx-auto -mt-7 max-w-[700px] px-6">
        <div className="relative">
          ...
          <input
            ...
            disabled={loading}
            className="w-full rounded-[14px] border-[1.5px] border-[#e2e8f0] bg-white py-4 pl-[52px] pr-24 text-[15px] text-[#0f172a] shadow-[0_8px_32px_rgba(0,0,0,.16)] outline-none transition focus:border-[#059669] disabled:opacity-60 dark:border-[#1e2d3d] dark:bg-[#131e2e] dark:text-[#f1f5f9] dark:shadow-[0_8px_32px_rgba(0,0,0,.2)]"
          />
          {searchQuery && (
      ...
      <section className="px-6 py-12 pb-[72px]">
        <div className="mx-auto max-w-[900px]">
          {/* Loading State */}
          {loading && (
            <div className="mb-8 flex items-center justify-center gap-2 text-[#64748b] dark:text-[#94a3b8]">
          ...
          <div className="mt-12 rounded-[20px] bg-[linear-gradient(135deg,#059669,#0891b2)] p-10 text-center text-white">
            ...
            <p className="mb-6 text-sm text-white/85">{t("ctaDesc")}</p>
            <div className="flex flex-wrap justify-center gap-3">
              <a
                href="https://wa.me/8801700262624"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 rounded-[10px] bg-[#25D366] px-[22px] py-3 text-sm font-bold text-white transition hover:scale-105 hover:shadow-lg"
              >
